...
for (i in c(1:repeats)) {
  ...
  train_data <- read.csv(glue("{path}/train_data_{i}.csv"))
  test_data <- read.csv(glue("{path}/test_data_{i}.csv"))
  ...
  # Output predictions
  if (overwrite) {
    write.csv(s$spec_pars$Estimate, file = glue("models/experiment_{exp_id}/brms_sigma_{i}.csv"), row.names = F)
    # write.csv(re, file = glue("models/experiment_{exp_id}/brms_re_{i}.csv"), row.names = F)
    # write.csv(y_pred_train, file = glue("models/experiment_{exp_id}/brms_pred_train_{i}.csv"), row.names = F)
    write.csv(y_pred_test, file = glue("models/experiment_{exp_id}/brms_pred_test_{i}.csv"), row.names = F)
  }
  ...
  # Output predictions
  if (overwrite) {
    write.csv(gp_model$get_cov_pars()[[1]], file = glue("models/experiment_{exp_id}/gpboost_scale_{i}.csv"), row.names = F)
    # write.csv(re, file = glue("models/experiment_{exp_id}/gpboost_re_{i}.csv"), row.names = F)
    # write.csv(y_pred_train, file = glue("models/experiment_{exp_id}/gpboost_pred_train_{i}.csv"), row.names = F)
    write.csv(y_pred_test, file = glue("models/experiment_{exp_id}/gpboost_pred_test_{i}.csv"), row.names = F)
  }
}
