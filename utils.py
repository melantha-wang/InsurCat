...
def evaluate_predictions(y, y_pred, categories, likelihood="gaussian", **kwargs):
    ...
    Likelihoods: "gaussian", "gamma", "lognorm", "loggamma".
    Specify the estimated parameters of the likelihood in `**kwargs` (to compute CRPS):
        - "gaussian" requires `loc` and `scale`;
        - "gamma" requires `shape` and `scale` (or synonyms `gamma_shape` and `gamma_scale`);
    ...
    if likelihood in ["gamma", "loggamma"]:
        ...
        if likelihood == "loggamma":
            ...
            y = np.exp(y)
            # Transform the predictions back to the original scale
            # y_pred = np.power(1 / (1 - gamma_scale), gamma_shape)
...
# https://stackoverflow.com/a/57439591
...
def plot_history(history, accuracy=False, plot_vertical=False):
    # Adapted from lecture code
    if accuracy:
        fig, ax = plt.subplots(
            nrows=2 if plot_vertical else 1, 
            ncols=1 if plot_vertical else 2,
            figsize=(7, 10) if plot_vertical else (14, 5))
        ax = ax.flatten()
        # Omit the first data point by taking [1:] for a better plot
        epochs = np.add(1, range(len(history["loss"]))[1:])
        ax[0].plot(epochs, history["accuracy"][1:], label="Training")
        ax[0].plot(epochs, history["val_accuracy"][1:], label="Validation")
        ax[0].legend(loc="lower right")
        ax[0].title("Accuracy")

        ax[1].plot(epochs, history["loss"][1:], label="Training")
        ax[1].plot(epochs, history["val_loss"][1:], label="Validation")
        ...
        # Omit the first data point by taking [1:] for a better plot
        epochs = np.add(1, range(len(history["loss"])[1:]))
        plt.plot(epochs, history["loss"][1:], label="Training")
        plt.plot(epochs, history["val_loss"][1:], label="Validation")
